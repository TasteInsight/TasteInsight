<template>
  ...
  >
    <img
      :src="(dish.images && dish.images[0]) || '/static/default_dish.png'"
      :alt="dish.name"
    ...
    <view class="flex-grow flex flex-col justify-between">
      ...
      <view class="flex justify-between items-center">
        ...
        <view class="flex items-center bg-yellow-50 px-2 py-1 rounded">
          ...
          <span class="text-yellow-600 ml-1 font-semibold text-sm">{{
            dish.averageRating === 0 ? '暂无' : dish.averageRating.toFixed(2)
          }}</span>
