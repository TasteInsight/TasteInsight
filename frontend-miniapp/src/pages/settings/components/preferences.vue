  ...
  <view class="w-full min-h-screen bg-gray-50 p-4 pb-10">
    ...
    <template v-else>
      ...
      <view class="bg-white rounded-lg p-6 mb-4 shadow-sm border border-gray-100">
        ...
        <view class="mb-8">
          <view class="flex justify-between items-center mb-3">
            ...
            <text class="text-sm text-ts-purple font-bold bg-purple-50 px-2 py-0.5 rounded">{{
              spicinessLabels[form.spiciness]
            }}</text>
          </view>
          <slider
          ...
          <view class="flex justify-between mt-2 px-1">
            <view
              v-for="(label, index) in spicinessLabels"
              :key="index"
        ...
        <view class="mb-8">
          ...
          <view class="flex justify-between mt-2 px-1">
            <view
              v-for="(label, index) in tasteLabels"
              :key="index"
              class="flex flex-col items-center"
              style="width: 16%"
            >
      ...
      <view class="bg-white rounded-lg p-6 mb-4 shadow-sm border border-gray-100">
        ...
        <view class="flex flex-wrap gap-2" v-if="form.favoriteIngredients.length > 0">
          ...
          >
            <text>{{ item }}</text>
            <view
              class="ml-2 w-4 h-4 rounded-full bg-purple-200 flex items-center justify-center"
              @click="removeFavoriteIngredient(index)"
            >
      ...
      <view class="bg-white rounded-lg p-6 mb-4 shadow-sm border border-gray-100">
        ...
        <picker
          :value="Object.values(portionLabels).indexOf(portionLabels[form.portionSize])"
          :range="Object.values(portionLabels)"
          @change="
            (e: any) =>
              (form.portionSize =
                reversePortionLabels[Object.values(portionLabels)[e.detail.value]])
          "
        >
          <view
            class="w-full p-3 border border-gray-200 rounded-lg flex justify-between items-center bg-white active:bg-gray-50 transition-colors"
          >
      ...
      <view class="bg-white rounded-lg p-6 mb-4 shadow-sm border border-gray-100">
        ...
        <view class="flex flex-wrap gap-2" v-if="form.meatPreference.length > 0">
          ...
          >
            ...
            <view
              class="ml-2 w-4 h-4 rounded-full bg-purple-200 flex items-center justify-center"
              @click="removeMeatPreference(index)"
            >
              <text class="text-xs text-purple-700 font-bold">×</text>
            </view>
          </view>
      ...
      <view class="bg-white rounded-lg p-6 mb-4 shadow-sm border border-gray-100">
        ...
        <view class="mb-4">
          ...
          >
            <view
              class="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl border border-gray-200"
            >
      ...
      <view class="bg-white rounded-lg p-6 mb-4 shadow-sm border border-gray-100">
        ...
        <view class="flex flex-wrap gap-2" v-if="form.avoidIngredients.length > 0">
          ...
          >
            <text>{{ item }}</text>
            <view
              class="ml-2 w-4 h-4 rounded-full bg-red-200 flex items-center justify-center"
              @click="removeAvoidIngredient(index)"
            >
