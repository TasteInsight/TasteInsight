    ...
    <view class="bg-white rounded-lg w-80 p-4" @tap.stop>
      ...
      <view class="mb-4">
        ...
        <picker :range="reportTypes" range-key="label" @change="handleTypeChange">
          <view
            class="border border-gray-300 rounded p-2 text-sm flex justify-between items-center"
          >
      ...
      <view class="flex justify-end gap-2">
        <button class="bg-gray-200 text-gray-700 px-4 py-1.5 rounded text-sm m-0" @tap="close">
          取消
        </button>
        <button class="bg-red-500 text-white px-4 py-1.5 rounded text-sm m-0" @tap="submit">
          提交
        </button>
      </view>
    </view>
