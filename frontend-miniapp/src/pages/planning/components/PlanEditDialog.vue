    ...
    <view class="bg-white rounded-2xl w-full max-w-2xl max-h-screen-80 flex flex-col" @tap.stop>
      ...
      <scroll-view scroll-y class="flex-1 p-5">
        ...
        <view class="mb-5">
          ...
          <picker mode="selector" :range="mealTimeOptions" range-key="label" @change="onMealTimeChange">
            <view class="w-full py-3 px-3 border border-gray-300 rounded-lg">
        ...
        <view class="mb-5">
          ...
          <view class="border border-gray-300 rounded-lg p-3">
            <view v-if="formData.dishes.length === 0" class="text-gray-400 text-sm text-center py-5">
              请点击"添加菜品"按钮选择菜品
            </view>
            <view v-else class="flex flex-wrap gap-2 mb-3">
              <view 
                v-for="(dishId, index) in formData.dishes" 
                :key="dishId"
                class="bg-gray-100 py-1.5 px-3 rounded-full flex items-center gap-2"
              >
                <text class="text-sm">菜品 {{ index + 1 }}</text>
                <text @tap="removeDish(index)" class="text-lg text-gray-500">×</text>
              </view>
            </view>
            <view 
              @tap="showDishSelector = true"
              class="w-full py-2 border border-dashed border-purple-700 bg-purple-50 rounded-lg text-center"
            >
      ...
      <view class="p-4 border-t border-gray-100 flex justify-end gap-3">
        <view @tap="handleClose" class="py-2 px-6 border border-gray-300 rounded-lg">
    ...
    <view v-if="showDishSelector" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-5" @tap="showDishSelector = false">
      <view class="bg-white rounded-2xl w-full max-w-md p-5" @tap.stop>
        ...
        <view class="flex justify-end gap-3">
          <view @tap="showDishSelector = false" class="py-2 px-6 border border-gray-300 rounded-lg">
            <text class="text-gray-600">取消</text>
          </view>
