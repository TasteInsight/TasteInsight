...
backend/src/
  ...
  // 管理端模块 (admin- 前缀)
  admin-dishes/
    - admin-dishes.module.ts
    - admin-dishes.controller.ts
    - admin-dishes.service.ts
    - dto/
      - admin-dish.dto.ts
      - admin-dish-batch.dto.ts
  admin-reviews/
  admin-canteens/
  admin-windows/
  admin-reports/
...
// 错误示例
@Controller('dishes')
export class dishesController {  // 错误：应使用PascalCase
  // ...
}

export class Dishes_Service {  // 错误：不应使用下划线
  // ...
}
```
...
export class AdminUpdateDishDto extends PartialType(AdminCreateDishDto) {}
...
export class DishListResponseDto {
  code: number;
  message: string;
  data: {
    items: DishDto[];
...
// 错误示例
export class dishDto {  // 错误：应使用PascalCase
  id: string;
}

export class Dish_ResponseDto {  // 错误：不应使用下划线
  code: number;
}
```

1. 【强制】接口和类型命名使用 PascalCase（不使用 I 前缀）

```typescript
// 正确示例 - TasteInsight 项目接口命名
...
// 错误示例
export interface IUserService {  // 错误：本项目不使用 I 前缀
  findById(id: string): Promise<User | null>;
}

export interface i_auth_payload {  // 错误：应使用 PascalCase
  userId: string;
}
```

1. 【强制】Guard 和 Decorator 命名遵循约定

```typescript
// 正确示例 - TasteInsight 项目 Guard 命名
// 文件: auth/guards/admin-auth.guard.ts
// 类名: AdminAuthGuard
...
// 正确示例 - Decorator 命名
// 文件: auth/decorators/permissions.decorator.ts
export const RequirePermissions = (...permissions: string[]) =>
  SetMetadata('permissions', permissions);

// 文件: auth/decorators/current-admin.decorator.ts
...
```typescript
// 正确示例 - TasteInsight 项目枚举定义 (common/enums.ts)
...
async updateDishStatus(
  ...
  return this.adminDishesService.updateDishStatus(id, updateStatusDto.status);
}
```
