...
frontend-miniapp/src/
  ...
  store/
    modules/
      - use-user-store.ts      // Store 文件使用 use-xxx-store.ts 命名
      - use-dishes-store.ts
      - use-canteen-store.ts
      - use-chat-store.ts
      - use-plan-store.ts
      - use-app-store.ts
...
// 小程序页面内组件
pages/dish/components/
  - AllCommentsPanel.vue
  - BottomReviewInput.vue
  - CommentList.vue
  - ReviewForm.vue
  - ReviewList.vue
  - TagList.vue
...
// 骨架屏组件
components/skeleton/
  - DishDetailSkeleton.vue
  - DishListSkeleton.vue
  - IndexSkeleton.vue
  - ProfileSkeleton.vue
  - SkeletonBase.vue
...
1. 【强制】接口命名使用 PascalCase（本项目不使用 I 前缀）；类型别名使用 PascalCase

```typescript
// 正确示例 - TasteInsight 项目类型定义 (types/api.d.ts)
...
export type DishStatus = 'online' | 'offline';

// 错误示例
export interface IUserInfo {  // 错误：本项目不使用 I 前缀
  id: string;
}

export type user_role = 'admin' | 'user';  // 错误：应使用 PascalCase
```
...
1. 【强制】UniApp 小程序页面和组件遵循 Vue 命名规范

```
// 正确示例 - TasteInsight 小程序项目结构
pages/
  index/                    # 首页
    - index.vue
    - components/
...
```typescript
// 正确示例 - TasteInsight 项目常量定义

// src/config/index.ts - 小程序配置
...
// 使用示例
import { APP_CONFIG, RecommendationScene } from '@/config';

const apiUrl = `${APP_CONFIG.API_BASE_URL}/dishes`;
await getRecommendations({ scene: RecommendationScene.HOME });
```

1. 【强制】Store 文件使用 `use-xxx-store.ts` 命名，采用 Composition API 风格

```typescript
// 正确示例 - TasteInsight 项目 Store 定义

// store/modules/use-user-store.ts
...
// store/modules/use-auth-store.ts (管理端)
...
1. 【强制】使用 Pinia 进行状态管理，采用 Composition API 风格

```typescript
// 正确示例 - TasteInsight 项目 Store 规范

// store/modules/use-dishes-store.ts - 小程序菜品 Store
import { defineStore } from 'pinia';
import { ref, computed } from 'vue';
...
export const useDishesStore = defineStore('dishes', () => {
  ...
  const topRatedDishes = computed(() => 
    [...dishes.value].sort((a, b) => b.averageRating - a.averageRating).slice(0, 3)
  );

  // Actions
  async function fetchDishes(params: GetDishesRequest) {
    loading.value = true;
    error.value = null;
  ...
  async function loadMore(params: GetDishesRequest) {
    ...
    try {
      const response = await getDishes({ ...params, page: currentPage.value });
      if (response.code === 200) {
        dishes.value = [...dishes.value, ...response.data.items];
        hasMore.value = response.data.meta.page < response.data.meta.totalPages;
      }
    } finally {
...
onMounted(async () => {
  await dishesStore.fetchDishes({ page: 1, pageSize: 10 });
});
</script>

<template>
  <view v-if="dishesStore.loading">加载中...</view>
  <view v-else>
...
2. 【强制】管理端使用权限 Store 统一管理权限

```typescript
// store/modules/use-auth-store.ts - 管理端认证 Store
import { defineStore } from 'pinia';
import { ref, computed } from 'vue';
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const login = async (credentials: LoginCredentials & { remember?: boolean }) => {
    ...
    if (response.code === 200 && response.data) {
      ...
      storage.setItem('admin_token', tokenInfo.accessToken);
      storage.setItem('admin_user', JSON.stringify(admin));
      storage.setItem('admin_permissions', JSON.stringify(userPermissions));
    }
  };
...
```typescript
// 正确示例 - TasteInsight 项目 API 模块化管理

// 小程序 API 导出风格 (导出独立函数)
// api/modules/dish.ts
import request from '@/utils/request';
import type { Dish, GetDishesRequest, PaginatedData, ApiResponse } from '@/types/api';
...
export const dishApi = {
  ...
  async getDishes(params?: GetDishesParams): Promise<ApiResponse<PaginationResponse<Dish>>> {
    return await request.get<ApiResponse<PaginationResponse<Dish>>>('/admin/dishes', { params });
  },
  ...
  async getDishById(id: string): Promise<ApiResponse<Dish>> {
    return await request.get<ApiResponse<Dish>>(`/admin/dishes/${id}`);
  },
  ...
  async createDish(dishData: DishCreateRequest): Promise<ApiResponse<Dish>> {
    return await request.post<ApiResponse<Dish>>('/admin/dishes', dishData);
  },
  ...
  async updateDish(id: string, dishData: DishUpdateRequest): Promise<ApiResponse<Dish>> {
    return await request.put<ApiResponse<Dish>>(`/admin/dishes/${id}`, dishData);
  },
  ...
  async deleteDish(id: string): Promise<ApiResponse<void>> {
    return await request.delete<ApiResponse<void>>(`/admin/dishes/${id}`);
  },
};
...
export * from './modules/canteen';
export * from './modules/meal-plan';
export * from './modules/ai';
export * from './modules/user';

// 使用示例 - 小程序
import { getDishes, favoriteDish } from '@/api';

const fetchDishes = async () => {
  const res = await getDishes({ page: 1, pageSize: 20 });
...
const fetchDishes = async () => {
  const res = await dishApi.getDishes({ page: 1, pageSize: 20 });
  dishes.value = res.data.items;
};
```
